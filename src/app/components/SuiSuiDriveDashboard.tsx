...
const SuiSuiDriveDashboard = () => {
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			...
			setDistanceTraveled(Number.parseFloat(newDistance.toFixed(3)));

			const newSuiSpent = newDistance * 0.01; // 1km = 0.01 SUI
			setSuiSpent(Number.parseFloat(newSuiSpent.toFixed(4)));

			// Update eco and safety scores
			setEcoScore(
				Math.min(100, Math.max(0, ecoScore + (Math.random() * 2 - 1))),
			);
			setSafetyScore(
				Math.min(100, Math.max(0, safetyScore + (Math.random() * 2 - 1))),
			);
		}, 1000);
		...
		<div className="w-full max-w-md mx-auto bg-gray-50 shadow-lg overflow-hidden min-h-screen flex flex-col">
			...
			<div className="bg-blue-600 text-white p-3 sticky top-0 z-10">
				<div className="flex items-center justify-between">
					...
					<div className="flex items-center">
						<div
							className={`flex items-center ${isLive ? "text-green-300" : "text-gray-300"}`}
						>
							<span
								...
								<div className="absolute inset-0 bg-gray-100">
									<div className="h-full w-full flex items-center justify-center">
										<div className="text-center">
											<MapPin
												size={24}
												className="mx-auto text-blue-300 mb-1"
											/>
											<p className="text-gray-500 text-xs">
												Loading map data...
											</p>
											<p className="text-gray-400 text-xs mt-1">
												Detailed location data is not recorded for privacy
											</p>
										</div>
									</div>
								</div>
				...
				{activeTab === "achievements" && (
					<div className="bg-white rounded-lg shadow">
						<div className="p-3 border-b">
							...
							<p className="text-xs text-gray-500">
								Unlock badges with safe driving
							</p>
						</div>
								...
								<table className="min-w-full">
									...
									<tbody className="divide-y divide-gray-100">
										{rankings.map((driver) => (
											<tr
												key={driver.rank}
												className={driver.name === "You" ? "bg-blue-50" : ""}
											>
